...
class GlobalGeMPool2d(nn.Module):
    """GeM pooling with one scalar exponent shared by every feature channel."""

    def __init__(self, p=1.0, p_min=1.0, p_max=4.0, eps=1e-6):
        super(GlobalGeMPool2d, self).__init__()
        if not (0.0 < p_min <= p <= p_max):
            raise ValueError("GeM requires 0 < p_min <= p <= p_max")
    ...
    def forward(self, x):
        # The last residual block is followed by ReLU, so its activations are
        # non-negative. Clamping only protects the exponent gradient at zero.
        p = self.p.clamp(min=self.p_min, max=self.p_max)
        x = x.clamp_min(self.eps).pow(p)
        return x.mean(dim=(-2, -1), keepdim=True).pow(p.reciprocal())

    @torch.no_grad()
    def project_p_(self):
        """Keep the stored/FedAvg exponent inside the valid search interval."""
        self.p.clamp_(min=self.p_min, max=self.p_max)

    def extra_repr(self):
        return (
            f"p={self.p.item():.4f}, p_min={self.p_min}, "
            f"p_max={self.p_max}, eps={self.eps}"
        )
...
class ResNet_PC(ResNetBase):
    def __init__(
        ...
        if use_global_gem:
            # A single learnable p is shared by all channels and, through the
            # state_dict/FedAvg path, by all federated clients. Starting from
            # p=1 reproduces average pooling up to the epsilon clamp.
            self.avgpool = GlobalGeMPool2d(p=1.0, p_min=1.0, p_max=4.0)
        else:
            # Keep every existing ResNet_PC caller on the original baseline.
            self.avgpool = nn.AvgPool2d(kernel_size=8)
        self.classifier = nn.Linear(
            in_features=int(64 * scaling * block_fn.expansion),
            out_features=self.num_classes,
        ...
        self.dim = self.classifier.in_features
