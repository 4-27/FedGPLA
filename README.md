...
Minimal PyTorch code for reproducing the main FedGPLA results in federated
semi-supervised learning. The release intentionally contains only the training
entry point and its direct model/data dependencies; datasets, checkpoints,
logs, exploratory variants, and intermediate experiment artifacts are omitted.
...
and prior-correction strength `0.25`. Dataset-specific communication rounds and
labeled samples per class are set by `FedGPLA.py`:

| Dataset | Rounds | Labeled/class |
| --- | ---: | ---: |
| CIFAR-10 | 300 | 500 |
| CIFAR-100 | 500 | 50 |
| SVHN | 150 | 460 |
